...
class TabLLViewController: UIViewController , UITabBarDelegate,UITableViewDataSource, CalendarPopUpDelegate, Protocol{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        var myobj = Utils.getbyloai_ll(items: bcth, loai: chon)
    ...
    func getdata ()
    {
        ...
        let url_vlr = "http://www.simmobi.vn:8090/QLCVMobiWebService/wsqlcv?cmd=113&userid=7592&ms_phongban=620&mucquyen=4&istrungtam=1&thang="+myFilter.thang+"&nam="+myFilter.nam+"&idtinh="+myFilter.tinh+"&loai=" + myFilter.loai
        ...
        let task = URLSession.shared.dataTask(with: urllink) { (data, _, _) in
            ...
            print(data)
            do {
                    let vlr = try JSONDecoder().decode(BcluuluongObj.self, from: data)
                self.list = vlr.bcluuluongInfo
                DispatchQueue.main.async {
                    self.mytableVlr.reloadData()
                    self.listloai.removeAll()
                    self.listloai.append("Tháng " + String(vlr.bcluuluongInfo[0].thang))
                    self.listloai.append("Quí " + String(vlr.bcluuluongInfo[0].quy))
                    self.listloai.append("Năm " + String(vlr.bcluuluongInfo[0].nam))
                    self.setupdoituong()
                }
            } catch {
                print("error getdata")
